...
def manageFaces(people, encoding):
   ...
   # matches = face_recognition.compare_faces(np.array(list([person.getPersonEncoding() for person in people])), encoding, tolerance=0.48)
    ...
    if b[0][1] < .53:
     ...
     #   first_match_index = matches.index(True)
      #  people[first_match_index].addEncoding(encoding)
       # name = people[first_match_index].getName()
    else:
